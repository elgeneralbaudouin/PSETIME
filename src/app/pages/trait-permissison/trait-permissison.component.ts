import { Component, OnInit } from '@angular/core';

@Component({
  selector: 'app-trait-permissison',
  templateUrl: './trait-permissison.component.html',
  styleUrls: ['./trait-permissison.component.css']
})
export class TraitPermissisonComponent implements OnInit {

  constructor() { }

  ngOnInit(): void {
  }

  public trairev: any[] = [
    {
      image3: "assets/img/OIP.jpg",
      Nom: "Nkodo Gaetan",
      Fonction: "Dev Front End",
      Id: 1,
      date: "02/09/2021",
      textobjet: "Demande d'annulation des absences du 12 Fevier 2021",
      justtexte1: "Ce jour nous eions en mission a Douala car nous travallions sur le projet du PAD",
      justetexte2: "",
    }
  ];
}
